...
//    CGRect *imageRect = [self _makeFitRect:logoImage.size];
    ...
    UIImageView *frontView = [[UIImageView alloc]initWithFrame:logoRect];
    frontView.image = [UIImage imageNamed:@"light_front.png"];
    frontView.alpha = 1.0;
...
-(void)initWithBrand:(Brand*)brand{
    ...
    NSArray* arr = [[BrandManager defaultManager] unreadImagesForBrand:brand];
...
-(void)itemImageClick{
    [ViewSwitcher switchToItemView:self.brandToShow];
